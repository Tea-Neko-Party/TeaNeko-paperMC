...
import org.zexnocs.teaneko.core.command.CommandScanner;
...
public final class PaperCommandService implements IPaperCommandService, ITeaNekoInitializer {
     ...
     * @param completionService Paper 自动补全服务
    ...
    public synchronized void registerAll(JavaPlugin plugin) {
        ...
        List<CommandBinding> preparedBindings;
        try {
            preparedBindings = prepareBindings(plugin);
    ...
    private List<CommandBinding> prepareBindings(JavaPlugin plugin) {
        ...
        for (var commandPair : commandBeans.values()) {
            ...
            CommandMapData commandMapData = commandScanner.getPrefixCommand(
                    coreMetadata.value()[0]
            );
            if (commandMapData == null) {
                throw new IllegalStateException("Core 指令扫描器中缺少主指令："
                        + coreMetadata.value()[0]);
            }
            var subCommandMetadata =
                    PaperCommandIntrospectionUtils.getMinecraftSubCommandMetadata(commandClass);
            completionService.prepareCommand(commandMapData, subCommandMetadata);
