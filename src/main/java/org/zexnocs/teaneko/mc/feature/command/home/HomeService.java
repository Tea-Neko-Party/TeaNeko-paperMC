...
public class HomeService {
     ...
     * 预加载玩家家名称到补全缓存。
     * <p>
     * 本方法可能访问数据库并等待读取完成，必须在异步线程调用。
     *
     * @param playerUuid 玩家 UUID
     */
    public void preloadHomeNames(UUID playerUuid) {
        Objects.requireNonNull(playerUuid, "玩家 UUID 不能为空。");
        synchronized (getPlayerLock(playerUuid)) {
            cacheHomeNames(playerUuid, getPlayerData(playerUuid));
        }
    }
     ...
     * 清理全部家名称补全快照。
     */
    public void clearCachedHomeNames() {
